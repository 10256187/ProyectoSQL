...
sql_tablas = [
    ...
    CREATE TABLE IF NOT EXISTS unitario (
        ...
        CONSTRAINT chk_unitario_unidad_valida CHECK (unidad IN ('UN','ML','KM','LB','KG','TN','SC','GL','GB','PG','M2','M3','Lt')),
        CONSTRAINT chk_unitario_codigo_formato CHECK (codigo REGEXP '^[A-Z]{4}-[0-9]{4}$')
    )
    """,
    """
    CREATE TABLE IF NOT EXISTS unitario_materiales (
        codigo_unitario VARCHAR(9),
        codigo_material VARCHAR(9),
        cantidad DECIMAL(10, 2) NOT NULL,
        PRIMARY KEY (codigo_unitario, codigo_material),
        FOREIGN KEY (codigo_unitario) REFERENCES unitario(codigo) ON DELETE CASCADE,
        FOREIGN KEY (codigo_material) REFERENCES materiales(codigo) ON DELETE CASCADE
    )
    """,
    """
    CREATE TABLE IF NOT EXISTS unitario_mobra (
        codigo_unitario VARCHAR(9),
        codigo_mobra VARCHAR(9),
        cantidad DECIMAL(10, 2) NOT NULL,
        PRIMARY KEY (codigo_unitario, codigo_mobra),
        FOREIGN KEY (codigo_unitario) REFERENCES unitario(codigo) ON DELETE CASCADE,
        FOREIGN KEY (codigo_mobra) REFERENCES mobra(codigo) ON DELETE CASCADE
    )
    """,
    """
    CREATE TABLE IF NOT EXISTS unitario_herramienta (
        codigo_unitario VARCHAR(9),
        codigo_herramienta VARCHAR(9),
        cantidad DECIMAL(10, 2) NOT NULL,
        PRIMARY KEY (codigo_unitario, codigo_herramienta),
        FOREIGN KEY (codigo_unitario) REFERENCES unitario(codigo) ON DELETE CASCADE,
        FOREIGN KEY (codigo_herramienta) REFERENCES herramienta(codigo) ON DELETE CASCADE
    )
    """,
    """
    CREATE TABLE IF NOT EXISTS unitario_transporte (
        codigo_unitario VARCHAR(9),
        codigo_transporte VARCHAR(9),
        cantidad DECIMAL(10, 2) NOT NULL,
        PRIMARY KEY (codigo_unitario, codigo_transporte),
        FOREIGN KEY (codigo_unitario) REFERENCES unitario(codigo) ON DELETE CASCADE,
        FOREIGN KEY (codigo_transporte) REFERENCES transporte(codigo) ON DELETE CASCADE
    )
    """
]
